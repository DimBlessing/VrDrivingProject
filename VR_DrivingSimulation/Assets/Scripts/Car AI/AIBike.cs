...
using UnityEngine;
    ...
    // car needs to be in.
    public float engineTorque = 50.0f;
    ...
    void FixedUpdate()
    {
        var kmh = rb.velocity.magnitude * 3.6f;
       ...
       rb.drag = rb.velocity.magnitude / 250f;
        ...
        engineRPM = (frontWheel.rpm) / 2 * gearRatio[currentGear];
        ShiftGears();

        // finally, apply the values to the wheels.	The torque applied is divided by the current gear, and
        // multiplied by the calculated AI input variable.
        frontWheel.motorTorque = engineTorque * 10f / gearRatio[currentGear] * inputTorque;

        rearWheel.brakeTorque = brakePower / 2f;
        
        // the steer angle is an arbitrary value multiplied by the calculated AI input.
        frontWheel.steerAngle = (steerPower) * inputSteer;
    }
    ...
    void ShiftGears()
    {
        ...
        if (engineRPM >= maxEngineRPM)
        {
            ...
            {
                if (frontWheel.rpm * gearRatio[i] < maxEngineRPM)
                {
                    appropriateGear = i;
                    break;
                }
        ...
        if (engineRPM <= minEngineRPM)
        {
            ...
            {
                if (frontWheel.rpm * gearRatio[j] > minEngineRPM)
                {
                    appropriateGear = j;
                    break;
                }
