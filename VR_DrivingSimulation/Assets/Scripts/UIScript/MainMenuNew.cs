...
namespace SlimUI.ModernMenu{
	public class MainMenuNew : MonoBehaviour {
		...
		private int level = 1;
		public void setLevel(int input_level)
		{
			level = 1;
			GameSetting.Instance.Level = input_level;
		}
		public int getLevel()
		{
			return level;
		...
		public void ExtrasMenu(){
			...
			if (extrasMenu)
			{
				if (extrasMenu.activeSelf == false) extrasMenu.SetActive(true);
				else extrasMenu.SetActive(false);
			}
			exitMenu.SetActive(false);
		}
		...
		IEnumerator LoadAsynchronously(){ // scene name is just the name of the current scene being loaded
